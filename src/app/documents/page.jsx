  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-neutral-800 dark:text-neutral-50 tracking-tight">Document Center</h1>
          <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-1">Access brochures, contracts, invoices, and templates</p>
        </div>
        <button 
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 border-b border-neutral-100 dark:border-neutral-800">
        {categories.map((cat) => (
          <button
            key={cat}
            ...
            className={`px-4 py-2 rounded-xl text-xs font-semibold cursor-pointer transition-all ${
              activeCategory === cat 
                ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-sky-400' 
                : 'text-neutral-500 hover:bg-neutral-50 dark:hover:bg-[#131b26]'
            }`}
          >
            {cat}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {filteredDocs.length === 0 ? (
          <div className="col-span-full text-center py-20 bg-white dark:bg-[#131b26] border rounded-3xl">
            <Folder className="w-12 h-12 text-neutral-300 mx-auto" />
            <h3 className="mt-4 text-xs font-semibold text-neutral-700 dark:text-neutral-300">No documents found</h3>
          ...
          filteredDocs.map((doc) => (
            <div 
              key={doc.id}
              className="bg-white dark:bg-[#131b26] border border-neutral-100 dark:border-neutral-800 p-5 rounded-2xl shadow-xs card-hover flex flex-col justify-between"
            >
              <div className="flex items-start gap-3">
                ...
                <div className="overflow-hidden">
                  <h4 className="font-semibold text-xs text-neutral-800 dark:text-neutral-200 truncate" title={doc.name}>
                    {doc.name}
                  </h4>
              ...
              <div className="flex items-center justify-between mt-6 pt-3 border-t border-neutral-50 dark:border-neutral-800/80 text-[10px] text-neutral-400">
                <span>Date: {doc.date}</span>
                <div className="flex gap-2">
                  <button 
                    onClick={() => toast.success(`Initiated download for ${doc.name}`)}
                    className="p-1 rounded hover:bg-neutral-50 dark:hover:bg-neutral-800 text-blue-600 dark:text-sky-400"
                    title="Download"
                  >
                  ...
                  <button 
                    ...
                    className="p-1 rounded hover:bg-neutral-50 dark:hover:bg-neutral-800 text-red-500"
                    title="Delete"
                  >
      ...
      {showUploadModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/45 backdrop-blur-xs">
          <div className="w-full max-w-sm bg-white dark:bg-[#131b26] border border-neutral-100 dark:border-neutral-800 rounded-3xl shadow-2xl p-6 relative">
            ...
            <h2 className="text-sm font-bold text-neutral-800 dark:text-neutral-50 mb-6">Upload Document</h2>
            ...
            <form onSubmit={handleUploadSubmit} className="space-y-4">
              <div className="space-y-1">
                ...
                <input 
                  ...
                  placeholder="CRM Sales Presentation" className="w-full p-2.5 rounded-xl border text-xs dark:bg-[#1c2635]"
                />
              </div>
              ...
              <div className="border-2 border-dashed border-neutral-200 dark:border-neutral-800 rounded-xl p-6 text-center cursor-pointer hover:bg-neutral-50/50">
