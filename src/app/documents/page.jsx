  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-gray-900 tracking-tight">Document Center</h1>
          <p className="text-xs text-gray-400 mt-1">Access brochures, contracts, invoices, and templates</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {filteredDocs.length === 0 ? (
          <div className="col-span-full text-center py-20 bg-white border rounded-3xl">
            ...
            <h3 className="mt-4 text-xs font-semibold text-gray-800">No documents found</h3>
            <p className="text-[10px] text-gray-400 mt-1">Upload files to get started.</p>
          </div>
        ) : (
          filteredDocs.map((doc) => (
            <div 
              key={doc.id}
              className="bg-white border border-gray-200 p-5 rounded-2xl shadow-sm card-hover flex flex-col justify-between"
            >
              <div className="flex items-start gap-3">
                ...
                <div className="overflow-hidden">
                  <h4 className="font-semibold text-xs text-gray-900 truncate" title={doc.name}>
                    {doc.name}
                  </h4>
                  <p className="text-[10px] text-gray-400 mt-0.5">{doc.category} • {doc.size}</p>
                </div>
              </div>

              <div className="flex items-center justify-between mt-6 pt-3 border-t border-neutral-50 text-[10px] text-gray-400">
                <span>Date: {doc.date}</span>
                <div className="flex gap-2">
                  <button 
                    onClick={() => toast.success(`Initiated download for ${doc.name}`)}
                    className="p-1 rounded hover:bg-gray-50 text-blue-600"
                    title="Download"
                  >
                  ...
                  <button 
                    ...
                    className="p-1 rounded hover:bg-gray-50 text-red-500"
                    title="Delete"
                  >
      ...
      {showUploadModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/45 backdrop-blur-xs">
          <div className="w-full max-w-sm bg-white border border-gray-200 rounded-3xl shadow-2xl p-6 relative">
            ...
            <h2 className="text-sm font-bold text-gray-900 mb-6">Upload Document</h2>

            <form onSubmit={handleUploadSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[9px] font-semibold text-gray-500 uppercase tracking-wide">Document Name *</label>
                <input 
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-semibold text-gray-500 uppercase tracking-wide">Category Folder *</label>
                <select 
              ...
              <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center cursor-pointer hover:bg-white">
                <Upload className="w-6 h-6 text-gray-400 mx-auto mb-2" />
                <span className="text-[10px] text-gray-500 block">Drag & Drop PDF or Click to Select File</span>
                <span className="text-[8px] text-gray-400 block mt-1">(Max File size: 20MB)</span>
              </div>
